/**
 * call-list-dispute-messages-admin-noauth.js
 *
 * CMD:
 *   cd /d C:\Users\teach\xdrivee-backend\functions
 *   node call-list-dispute-messages-admin-noauth.js <disputeId> [limit]
 */
...
async function main() {
  ...
  if (disputeId === undefined) {
    console.error("USAGE: node call-list-dispute-messages-admin-noauth.js <disputeId> [limit]");
    process.exit(1);
  }
  ...
  console.log("=== Client test: listDisputeMessagesAdminEU [no auth] ===");
  console.log("disputeId:", disputeId);
  if (rawLimit !== undefined) console.log("limit:", payload.limit);

  const app = initializeApp(firebaseConfig);
  const functions = getFunctions(app, "europe-west1");
  const fn = httpsCallable(functions, "listDisputeMessagesAdminEU");
