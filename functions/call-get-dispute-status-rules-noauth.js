/**
 * call-get-dispute-status-rules-noauth.js
 *
 * CMD:
 *   cd /d C:\Users\teach\xdrivee-backend\functions
 *   node call-get-dispute-status-rules-noauth.js
 */
...
async function main() {
  console.log("=== Client test: getDisputeStatusRulesEU [no auth] ===");

  const app = initializeApp(firebaseConfig);
  const functions = getFunctions(app, "europe-west1");
  const fn = httpsCallable(functions, "getDisputeStatusRulesEU");
  const res = await fn({});
