/**
 * call-list-dispute-actions-noauth.js
 *
 * CMD:
 *   cd /d C:\Users\teach\xdrivee-backend\functions
 *   node call-list-dispute-actions-noauth.js <disputeId>
 */
...
async function main() {
  ...
  if (disputeId === undefined) {
    console.error("USAGE: node call-list-dispute-actions-noauth.js <disputeId>");
    process.exit(1);
  }

  console.log("=== Client test: listDisputeActionsEU [no auth] ===");
  console.log("disputeId:", disputeId);

  const app = initializeApp(firebaseConfig);
  const functions = getFunctions(app, "europe-west1");
  const fn = httpsCallable(functions, "listDisputeActionsEU");
