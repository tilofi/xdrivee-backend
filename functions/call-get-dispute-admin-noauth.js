/**
 * call-get-dispute-admin-noauth.js
 *
 * CMD:
 *   cd /d C:\Users\teach\xdrivee-backend\functions
 *   node call-get-dispute-admin-noauth.js <disputeId>
 */
...
async function main() {
  ...
  if (disputeId === undefined) {
    console.error("USAGE: node call-get-dispute-admin-noauth.js <disputeId>");
    process.exit(1);
  }

  console.log("=== Client test: getDisputeAdminEU [no auth] ===");
  console.log("disputeId:", disputeId);

  const app = initializeApp(firebaseConfig);
  const functions = getFunctions(app, "europe-west1");
  const fn = httpsCallable(functions, "getDisputeAdminEU");
