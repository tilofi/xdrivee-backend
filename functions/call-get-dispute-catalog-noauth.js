/**
 * call-get-dispute-catalog-noauth.js
 *
 * CMD:
 *   cd /d C:\Users\teach\xdrivee-backend\functions
 *   node call-get-dispute-catalog-noauth.js [role]
 */
...
async function main() {
  ...
  console.log("=== Client test: getDisputeCatalogEU [no auth] ===");
  if (role !== undefined) console.log("role:", role);

  const app = initializeApp(firebaseConfig);
  const functions = getFunctions(app, "europe-west1");
  const fn = httpsCallable(functions, "getDisputeCatalogEU");
  const payload = {};
